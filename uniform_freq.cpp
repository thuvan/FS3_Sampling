...
int main(int argc, char *argv[]) {
  ...
  while (cur_iter<=max_iter){
    ...
    if (h!=NULL){
      ...
      cout<<*h;
      cout<<" code = "<<h->get_canonical_code().to_string()<<endl;
      cout<<"------------"<<endl;
    }
    ...
    int qItem_index = Q.findIndexByGraph(h);
    if (qItem_index!=-1)
    {
      cout<<"sampled subgraph existed at "<<qItem_index<<"\n";
      Queue_Item* qItem = Q.getItemAt(qItem_index);
      ///TODO: hoi ngu: sao ko thay update h_score?
      if (qItem->score<h_score)
        qItem->score = h_score;
      int preSupport = qItem->idset.size();
      int idx = find_in_vector(qItem->idset,graph_id);
//      cout<<"graph_id = "<<graph_id<<", \t Idset = ";
//      print_vector(qItem->idset);
//      cout<<"index in idset: "<<idx<<endl;
      if (idx==-1){
        qItem->idset.push_back(graph_id);
      }
      qItem->insert_time = cur_iter;
      //resort queue
      Q.update_priority_of_element(qItem_index);
    }
    else
    {
      cout<<"sampled subgraph is new\n";
      if (Q.isFull())
        Q.evictLast();
      Queue_Item* qItem = new Queue_Item();
      qItem->subgraph = h;
      qItem->idset.push_back(graph_id);
      qItem->insert_time = cur_iter;
      ///TODO: score cua subgraph o moi graph co the khac nhau => score luu score o graph nao?
