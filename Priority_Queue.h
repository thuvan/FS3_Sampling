...
class Priority_Queue
{
  ...
  public:
    ...
    void update_priority_of_element(int index){
      assert(index>=0 && index <_data.size());
      Queue_Item* item = _data[index];
      _data.erase(_data.begin()+index);
      push(item);
    }

    Queue_Item* getItemAt(int index){
      assert(index>=0 && index <_data.size());
      return _data[index];
    }

    int findIndexByGraph(PAT* g)
    {
      string code = g->get_canonical_code().to_string();
      for(int i=0;i<_data.size();i++){
        if (code.compare(_data[i]->subgraph->get_canonical_code().to_string())==0)
          return i;
      }
      return -1;
    }
